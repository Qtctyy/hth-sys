...
function sortKey(ride, now, today) {
  ...
  if (ride.to_work_time) {
    const done = ride.to_work_completed_date === today;
    legs.push({ done, time: ride.to_work_time });
  }
  if (ride.way_back_enabled && ride.way_back_time) {
    const done = ride.way_back_completed_date === today;
    legs.push({ done, time: ride.way_back_time });
  }
  const pending = legs.filter((l) => !l.done);
  if (pending.length === 0) return [1, Infinity];
  const soonest = Math.min(...pending.map((l) => minutesUntil(l.time, now)));
  return [0, soonest];
}
...
function RidesPageInner() {
  ...
  const [formAgent, setFormAgent] = useState(agent || AGENTS[0]);
  ...
  useEffect(() => {
    if (session && agent) loadRides();
  }, [session, agent, loadRides]);
  ...
  function startEdit(ride) {
    ...
    setFormAgent(ride.agent || agent || AGENTS[0]);
  ...
  return (
    <div className="wrap">
      <div className="topbar">
        <div>
          <button className="link" onClick={() => router.push('/')} style={{ marginBottom: 6 }}>← All agents</button>
          <div className="eyebrow">{agent}'s Rides</div>
          <div className="date-bar" style={{ marginBottom: 0 }}>{dateLabel}</div>
        </div>
        <button className="link" onClick={handleSignOut}>Sign out</button>
      </div>

      <h1 style={{ marginTop: 16 }}>Add a ride</h1>
      <div className="sub" style={{ marginBottom: 16 }}>Fill it in, hit save. Rides repeat daily until you delete them.</div>
      ...
      <form className="card" onSubmit={handleSave}>
        ...
        <input value={form.mobile_number} onChange={(e) => updateField('mobile_number', e.target.value)} placeholder="07XXXXXXXX" />
      ...
      {loadingRides ? (
        ...
        <div className="empty">No rides yet. Add the first one above.</div>
      ) : (
        sorted.map((r) => {
...
export default function RidesPage() {
  return (
    <Suspense fallback={<div className="center-screen"><div className="loading">Loading...</div></div>}>
      <RidesPageInner />
    </Suspense>
  );
}
